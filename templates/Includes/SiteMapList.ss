...
<% loop LevelOneSiteMapPages %>
	<li class="$FirstLast<% if SiteMapPages.count %><% else %> noChildrenLI<% end_if %>">
		<% if SiteMapPages.count %><a href="#" class="siteMapPageExpander SiteMapNodeImploded SiteMapLevel1" data-id="$ID">+</a> <% end_if %>
		...
		<ul id="sublist{$ID}" class="SubListLevel2"><% loop SiteMapPages %>
			<li class="$FirstLast<% if SiteMapPages.count %><% else %> noChildrenLI<% end_if %>">
				<% if SiteMapPages.count %><a href="#" class="siteMapPageExpander SiteMapNodeImploded SiteMapLevel2" data-id="$ID">+</a> <% end_if %>
				<a href="$Link" class="pageTitle SiteMapLevel2">$Title</a>
				<% if SiteMapPages.count %>
				<ul id="sublist{$ID}" class="SubListLevel3"><% loop SiteMapPages %>
					<li class="$FirstLast<% if SiteMapPages.count %><% else %> noChildrenLI<% end_if %>">
						<% if SiteMapPages.count %><a href="#" class="siteMapPageExpander SiteMapNodeImploded SiteMapLevel3" data-id="$ID">+</a> <% end_if %>
						<a href="$Link" class="pageTitle SiteMapLevel3">$Title</a>
						<% if SiteMapPages.count %>
						<ul id="sublist{$ID}" class="SubListLevel4"><% loop SiteMapPages %>
							<li class="$FirstLast<% if SiteMapPages.count %><% else %> noChildrenLI<% end_if %>">
								<% if SiteMapPages.count %><a href="#" class="siteMapPageExpander SiteMapNodeImploded SiteMapLevel4" data-id="$ID">+</a> <% end_if %>
